...
import { $, $$, untrack, useEffect, useEventListener } from 'vitro'
import type {
  F,
  OR,
  RouterNavigate,
...
const browser = (
  ...
  const navigate = (
    pathNext: RouterPath,
    navigateOptions: RouterNavigateOptions = {},
  ): void => {
  ...
  const scrollTo = (path: RouterPath): void => {
    // Scroll to the fragment manually
  ...
  if (options.history) {
    if (options.historyHash) {
      setTimeout(() => {
        //TODO: Wait for the page to be loaded better

        scrollTo(path())
      })

      useEventListener(globalThis.document, 'click', (event: MouseEvent) => {
        // Handle clicks on a[href="#"] elements
    ...
    useEventListener(globalThis.window, 'popstate', () => {
      // Update path from history
