...
import { h, untrack, useMemo, useResource } from 'vitro'
...
const Router = ({
  ...
  const lookup = useMemo(
    () => router.route(pathname()) || router.route('/404') || FALLBACK_ROUTE,
  )
  ...
  const loader = useMemo(() =>
    useResource(() => (route().loader || NOOP)(untrack(loaderContext))),
  )
